...
# 构建Q网络结构
dim = 256
class DQN(nn.Module): # DuelingDQN
    ...
    def forward(self, x):
        ...
        v = self.value(x)
        a = self.advantage(x)
        return v + (a - a.mean(dim=-1, keepdim=True))  # Q(s, a) = V(s) + A(s, a) - mean(A)
...
def train():
    ...
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.title("Training Curve (Dueling-DQN)")
    plt.legend()
    plt.grid(True)
...
=== 训练完成 ===
平均奖励值（最后50轮）: 249.14
收敛速度（达到平均奖励500所需轮数）: 92
稳定性（最后100轮标准差）: 141.00
'''
